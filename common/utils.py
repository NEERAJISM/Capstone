...
class Utils:
    ...
    def extract_archive_at_source(self, archive_path: str) -> None:
        ...
        if suffix == ".zip":
            ...
            try:
                ...
                raise RuntimeError(
                    "This is part of a split archive (e.g., .part1.rar). Please extract manually."
                )
            except rarfile.RarCannotExec as e:
                raise RuntimeError(
                    "Missing 'unrar' or 'bsdtar'. Please install it to handle RAR files."
                ) from e
    ...
    def load_stock_csv(filepath):
        ...
        if ccol is None:
            raise ValueError(
                f"Close column not found in {filepath}. Columns={df.columns.tolist()}"
            )
