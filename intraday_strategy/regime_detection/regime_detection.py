...
def save_regime_detected(trading_date, ticker):
    ...
    print("Trading day time range:", trading_data["datetime"].min(), "to", trading_data["datetime"].max())
    print("Training day time range:", training_data["datetime"].min(), "to", training_data["datetime"].max())
    ...
    result = detect_regimes_train_test_rolling(group, train_start, train_end, test_day, lookback=60)
